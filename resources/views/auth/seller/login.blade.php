    ...
    <div
        ...
        <div class="w-full bg-[#ECFDF5] p-1.5 md:p-2.5 rounded-lg flex gap-2 md:gap-3">
            <a href="{{ route('login.pembeli') }}"
                class="flex-1 rounded-md p-2 md:p-3 flex flex-col items-center justify-center gap-1.5 md:gap-2 transition cursor-pointer bg-[#D1FAE5] text-[#475569] hover:bg-[#A7F3D0]">
                <x-heroicon-o-shopping-cart class="w-5 h-5 md:w-6 md:h-6" />
                <span class="font-bold text-xs md:text-sm">Pembeli</span>
            </a>

            <a href="{{ route('seller.login') }}"
                class="flex-1 rounded-md p-2 md:p-3 flex flex-col items-center justify-center gap-1.5 md:gap-2 transition cursor-pointer bg-[#0F4C20] text-white shadow-sm">
        ...
        <form action="{{ route('seller.login.submit') }}" method="POST" class="w-full flex flex-col gap-4 md:gap-5">
            @csrf
            ...
            <button type="submit"
                class="mt-1 md:mt-2 w-full bg-[#0F4C20] hover:bg-[#0b3a18] text-white font-bold py-3 md:py-3.5 px-6 rounded-lg flex items-center justify-center gap-2 transition shadow-md text-sm md:text-base">
                Login Sekarang
                <x-heroicon-m-arrow-right class="h-4 w-4 md:h-5 md:w-5" />
            </button>
        </form>

        <div class="text-center text-[11px] md:text-sm text-gray-500 font-medium">
            Belum Punya Akun?
