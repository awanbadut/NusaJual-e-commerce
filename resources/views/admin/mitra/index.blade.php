            ...
            <tr class="border-b border-[#E5E7EB] hover:bg-white transition">
                <td class="px-5 py-4">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-11 h-11 {{ $colorClass }} rounded-full flex items-center justify-center font-bold text-[15px]">
                            {{ strtoupper(substr($store->store_name, 0, 2)) }}
                        </div>
                        <div>
                            <p class="font-semibold text-[#111827] text-[14px]">{{ $store->store_name }}</p>
                            <p class="text-[11px] text-[#9CA3AF]">ID: MT-{{ str_pad($store->id, 5, '0', STR_PAD_LEFT) }}
                            </p>
                        </div>
                    </div>
                </td>
                ...
                <td class="px-5 py-4">
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full bg-[#FBBF24] text-white text-[12px] font-semibold">
...
<div class="mt-4 flex justify-center gap-2">
    ...
    <a href="{{ $stores->previousPageUrl() }}"
        class="px-3 py-1.5 rounded-lg border border-[#D1D5DB] hover:bg-[#F3F4F6] text-[#111827] text-[12px]">‹</a>
    @endif
    ...
    @else
    <a href="{{ $url }}"
        class="px-3 py-1.5 rounded-lg border border-[#D1D5DB] hover:bg-[#F3F4F6] text-[#111827] text-[12px]">{{ $page
        }}</a>
    @endif
    @endforeach
