<x-layouts.profile title="Alamat Saya - Nusa Belanja" headerTitle="Alamat Saya"
    ...
    <div x-data="locationData()" x-init="fetchProvinces()">
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm relative flex flex-col gap-4">
                ...
                <div class="flex justify-between items-start">
                    ...
                    <span class="bg-[#563B1F] text-white text-[10px] font-bold px-3 py-1 rounded-full">
                        Utama
                    </span>
                    @else
                    <form action="{{ route('profile.address.setPrimary', $address->id) }}" method="POST">
                        ...
                        @method('PATCH')
                        <button type="submit"
                            class="bg-[#FEF9C3] hover:bg-[#fef08a] text-[#854D0E] text-[10px] font-bold px-3 py-1 rounded-full transition cursor-pointer">
                            Jadikan Utama
                        </button>
                    </form>
                    @endif
            ...
            <div class="text-center py-12 bg-white border border-dashed border-gray-300 rounded-xl">
                ...
                <p class="text-gray-500 text-sm mb-6">Tambahkan alamat pengiriman agar belanja lebih mudah.</p>
                <button @click="openModal()" class="text-[#0F4C20] font-bold text-sm hover:underline">
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-xl transition-all sm:my-8 w-full max-w-2xl border-t-4 border-[#0F4C20]"
                    ...
                    <form :action="isEdit ? updateUrl : storeUrl" method="POST" class="p-6 md:p-8 flex flex-col gap-5">
                        ...
                        <div
                            class="border border-gray-300 rounded-lg p-2.5 flex items-center justify-center gap-2 text-gray-500 bg-gray-50 cursor-not-allowed">
                            <x-heroicon-o-map-pin class="w-5 h-5" />
                            <span class="text-xs font-medium">Pilih lokasi dari peta (Segera Hadir)</span>
                        </div>
                        <p class="text-xs text-gray-400 text-center -mt-2">Pin lokasi Anda di peta untuk pengiriman yang
                            lebih akurat.</p>
                        ...
                        <div class="mt-4 flex justify-end gap-3 pt-6 border-t border-gray-100">
                            <button type="button" @click="showModal = false"
                                class="px-8 py-3 rounded-lg bg-[#F3F4F6] text-[#1F2937] font-bold text-sm hover:bg-gray-200 transition">
                                Batal
                            </button>
                            <button type="submit"
                                class="px-8 py-3 rounded-lg bg-[#0F4C20] text-white font-bold text-sm hover:bg-[#0b3a18] transition shadow-md">
                                Simpan Alamat
                            </button>
                        </div>
                    </form>
    ...
    function locationData() {
        return {
            ...
            async editAddress(address) {
                this.isEdit = true;
                this.updateUrl = `/profile/address/${address.id}`; // Pastikan route update ada
                ...
                // Load wilayah berurutan (Chain fetch agar dropdown terisi otomatis)
                // 1. Set Provinsi
                this.form.province_code = address.province_code;
                await this.fetchCities(); // Tunggu kota loading
                
                // 2. Set Kota
                this.form.city_code = address.city_code;
                await this.fetchDistricts(); // Tunggu kecamatan loading
                
                // 3. Set Kecamatan
                this.form.district_code = address.district_code;
                await this.fetchVillages(); // Tunggu kelurahan loading
            ...
            async fetchVillages() {
                ...
                // LIHAT DATA DI CONSOLE BROWSER (Tekan F12 -> Console)
            ...
            // setPostalCode() {
            //     // 1. Cari object data kelurahan yang sedang dipilih
            //     let selectedData = this.villages.find(item => item.code == this.form.village_code);

            //     // 2. Cek apakah data ditemukan DAN punya array postal_codes
            //     if (selectedData && selectedData.postal_codes && selectedData.postal_codes.length > 0) {
                    
            //         // 3. AMBIL INDEX 0 (Ini kuncinya, karena datanya array ["12345"])
            //         this.form.postal_code = selectedData.postal_codes[0];
                    
            //     } else {
            //         // 4. Jika tidak ada, kosongkan agar user bisa ketik manual
