<x-layouts.profile title="Pesanan Saya - Nusa Belanja" headerTitle="Pesanan Saya"
    ...
    <div class="flex overflow-x-auto pb-4 mb-6 gap-3 no-scrollbar border-b border-gray-100">
        ...
        <a href="{{ route('profile.orders', ['status' => $key]) }}" 
           class="whitespace-nowrap px-5 py-2.5 rounded-full text-sm font-bold border transition duration-200
            {{ $currentStatus == $key 
        ...
        <div x-data="{ open: false }"
            ...
            <div class="p-5 flex flex-col sm:flex-row gap-5 items-center sm:items-start" x-show="!open">
                ...
                <div class="flex-1 w-full text-center sm:text-left">
                    ...
                    <h4 class="font-bold text-[#2E3B27] text-base line-clamp-1 mb-1">{{ $firstItem->product->name }}</h4>
                ...
                <div class="text-right flex flex-col items-center sm:items-end justify-center h-full w-full sm:w-auto border-t sm:border-t-0 border-gray-100 pt-4 sm:pt-0 mt-2 sm:mt-0">
                    ...
                    <span class="text-lg font-bold text-[#0F4C20]">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div x-show="open" x-collapse class="bg-gray-50 border-t border-gray-200 p-6 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex gap-4 p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div class="flex-1">
                            <p class="text-[10px] font-bold text-gray-400 uppercase">{{ $item->product->category->name ?? 'Umum' }}</p>
                            <h5 class="font-bold text-[#2E3B27] text-sm line-clamp-1">{{ $item->product->name }}</h5>
                            <p class="text-xs text-[#8B4513] font-bold mt-1">{{ $item->quantity }} x {{ $item->product->unit }}</p>
                        </div>
                        <div class="text-right flex flex-col justify-center">
                            <span class="text-sm font-bold text-gray-600">Rp {{ number_format($item->total, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm h-full flex flex-col">
                        ...
                        <div class="text-xs text-gray-600 space-y-1 flex-1">
                            <p class="font-bold text-gray-800 text-sm">{{ $order->recipient_name }} <span class="font-normal text-gray-500">({{ $order->recipient_phone }})</span></p>
                ...
                <div class="flex gap-3 items-center">
                    ...
                    <div x-data="{ showModal: false }">
                        ...
                        <div x-show="showModal" 
                            ...
                            <div @click.stop class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
