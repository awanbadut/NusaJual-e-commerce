<x-layouts.profile title="Pesanan Saya - Nusa Belanja" headerTitle="Pesanan Saya"
    ...
    {{-- Status Filter Tabs --}}
    <div class="flex overflow-x-auto pb-2 md:pb-4 mb-4 md:mb-6 gap-2 md:gap-3 no-scrollbar border-b border-gray-100 w-full">
        @php
        $statuses = [
            'all'        => 'Semua',
        ...
        <a href="{{ route('profile.orders', ['status' => $key]) }}"
            class="whitespace-nowrap px-4 py-2 md:px-5 md:py-2.5 rounded-full text-xs md:text-sm font-bold border transition duration-200 flex-shrink-0
               {{ $currentStatus == $key
    ...
    <div class="flex flex-col gap-6">
        ...
        <div x-data="{ open: false }"
            ...
            {{-- Order Header --}}
            <div class="bg-white border-b border-gray-100 p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <span class="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Nomor Pesanan</span>
            ...
            <div class="p-5 flex flex-col sm:flex-row gap-5 items-center sm:items-start" x-show="!open">
                ...
                <div class="flex-1 w-full text-center sm:text-left">
                    <p class="text-xs font-bold text-gray-400 mb-1 uppercase tracking-wide">
                        {{ ($firstItem && $firstItem->product) ? ($firstItem->product->store->store_name ?? 'Nusa Belanja') : 'Nusa Belanja' }}
                    </p>
                    <h4 class="font-bold text-[#2E3B27] text-base line-clamp-1 mb-1">
                ...
                <div class="text-right flex flex-col items-center sm:items-end justify-center h-full w-full sm:w-auto border-t sm:border-t-0 border-gray-100 pt-4 sm:pt-0 mt-2 sm:mt-0">
            ...
            <div x-show="open" x-collapse class="bg-gray-50 border-t border-gray-200 p-6 space-y-6">
                ...
                <div class="mb-12 px-4 py-6 bg-white rounded-xl border border-gray-100 shadow-sm">
                    <div class="relative">
                        ...
                        @php
                        $progressWidth = '0%';
                        $steps = ['pending' => false, 'confirmed' => false, 'packing' => false, 'shipped' => false, 'completed' => false];
                        ...
                        <div class="relative z-10 flex justify-between w-full">
                            ...
                            <div class="flex flex-col items-center">
                                <div class="w-5 h-5 rounded-full border-4 border-white shadow-md transition-colors duration-500 {{ $steps['pending'] ? $activeColor : 'bg-gray-200' }}">
                                    @if($steps['pending'] && !$steps['confirmed'])
                                    <div class="w-full h-full rounded-full animate-ping {{ $activeColor }} opacity-20"></div>
                                    @endif
                                </div>
                                <div class="text-center mt-3">
                                    <p class="text-[10px] sm:text-xs font-black uppercase tracking-tight {{ $steps['pending'] ? $textColor : 'text-gray-400' }}">Dibuat</p>
                                    <p class="text-[9px] font-medium text-gray-400 mt-0.5 bg-gray-50 px-1.5 py-0.5 rounded-md inline-block">{{ $order->created_at->format('d/m H:i') }}</p>
                                </div>
                            </div>

                            {{-- Step: Dikonfirmasi --}}
                            <div class="flex flex-col items-center">
                                <div class="w-5 h-5 rounded-full border-4 border-white shadow-md transition-colors duration-500 {{ $steps['confirmed'] ? $activeColor : 'bg-gray-200' }}">
                                    @if($steps['confirmed'] && !$steps['packing'])
                                    <div class="w-full h-full rounded-full animate-ping {{ $activeColor }} opacity-20"></div>
                                    @endif
                                </div>
                                <div class="text-center mt-3">
                                    <p class="text-[10px] sm:text-xs font-black uppercase tracking-tight {{ $steps['confirmed'] ? $textColor : 'text-gray-400' }}">Dikonfirmasi</p>
                                    @if($order->payment && $order->payment->confirmed_at)
                                    <p class="text-[9px] font-medium text-gray-400 mt-0.5 bg-gray-50 px-1.5 py-0.5 rounded-md inline-block">{{ $order->payment->confirmed_at->format('d/m H:i') }}</p>
                                    @endif
                                </div>
                            </div>
                            ...
                            {{-- Step: Dikemas --}}
                            <div class="flex flex-col items-center">
                                <div class="w-5 h-5 rounded-full border-4 border-white shadow-md transition-colors duration-500 {{ $steps['packing'] ? $activeColor : 'bg-gray-200' }}">
                                    @if($steps['packing'] && !$steps['shipped'])
                                    <div class="w-full h-full rounded-full animate-ping {{ $activeColor }} opacity-20"></div>
                                    @endif
                                </div>
                                <div class="text-center mt-3">
                                    <p class="text-[10px] sm:text-xs font-black uppercase tracking-tight {{ $steps['packing'] ? $textColor : 'text-gray-400' }}">Dikemas</p>
                                    <p class="text-[9px] font-medium text-gray-300 mt-0.5 italic">Proses Seller</p>
                                </div>
                            </div>

                            {{-- Step: Dikirim --}}
                            <div class="flex flex-col items-center">
                                <div class="w-5 h-5 rounded-full border-4 border-white shadow-md transition-colors duration-500 {{ $steps['shipped'] ? $activeColor : 'bg-gray-200' }}">
                                    @if($steps['shipped'] && !$steps['completed'])
                                    <div class="w-full h-full rounded-full animate-ping {{ $activeColor }} opacity-20"></div>
                                    @endif
                                </div>
                                <div class="text-center mt-3">
                                    <p class="text-[10px] sm:text-xs font-black uppercase tracking-tight {{ $steps['shipped'] ? $textColor : 'text-gray-400' }}">Dikirim</p>
                                    @if($order->shipped_at)
                                    <p class="text-[9px] font-medium text-gray-400 mt-0.5 bg-gray-50 px-1.5 py-0.5 rounded-md inline-block">{{ $order->shipped_at->format('d/m H:i') }}</p>
                                    @endif
                                </div>
                            </div>
                            ...
                            {{-- Step: Selesai --}}
                            <div class="flex flex-col items-center">
                                <div class="w-5 h-5 rounded-full border-4 border-white shadow-md transition-colors duration-500 {{ $steps['completed'] ? $activeColor : 'bg-gray-200' }}"></div>
                                <div class="text-center mt-3">
                                    <p class="text-[10px] sm:text-xs font-black uppercase tracking-tight {{ $steps['completed'] ? $textColor : 'text-gray-400' }}">Selesai</p>
                                    @if($order->delivered_at)
                                    <p class="text-[9px] font-medium text-gray-400 mt-0.5 bg-gray-50 px-1.5 py-0.5 rounded-md inline-block">{{ $order->delivered_at->format('d/m H:i') }}</p>
                                    @endif
                                </div>
                            </div>
                ...
                <div class="mb-6 p-4 bg-red-50 border border-red-100 rounded-lg">
                    ...
                    <div x-data="{ showBankForm: false }" class="mt-4 p-4 bg-yellow-50 border border-yellow-300 rounded-xl">
                        <div class="flex items-start gap-3 mb-3">
                            ...
                            <div>
                                <p class="text-sm font-bold text-yellow-800">Harap isi rekening untuk pengembalian dana</p>
                                <p class="text-xs text-yellow-700 mt-1">
                                    Pesanan dibatalkan oleh penjual. Masukkan rekening bank Anda agar admin dapat mentransfer
                                    <strong>Rp {{ number_format($order->refund->refund_amount, 0, ',', '.') }}</strong>
                                    (sudah dipotong biaya admin 5%).
                                </p>
                            </div>
                        </div>

                        <button @click="showBankForm = !showBankForm"
                            class="w-full py-2 rounded-lg bg-yellow-500 text-white text-sm font-bold hover:bg-yellow-600 transition">
                            <span x-text="showBankForm ? '▲ Tutup Form' : '▼ Isi Rekening Sekarang'"></span>
                        </button>

                        <form x-show="showBankForm" x-cloak
                              action="{{ route('profile.orders.refund.bank', $order->id) }}"
                              method="POST"
                            ...
                            <div class="p-3 bg-white rounded-lg border border-yellow-200 space-y-1.5 text-xs">
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-semibold">Rp {{ number_format($order->refund->order_amount, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-500">Biaya Admin (5%)</span>
                                    <span class="font-semibold text-red-600">- Rp {{ number_format($order->refund->admin_fee, 0, ',', '.') }}</span>
                                </div>
                                <hr class="border-yellow-200">
                                <div class="flex justify-between">
                                    <span class="font-bold text-gray-800">Dana Dikembalikan</span>
                                    <span class="font-bold text-green-700">Rp {{ number_format($order->refund->refund_amount, 0, ',', '.') }}</span>
                                </div>
                            </div>
                    ...
                    {{-- Status Refund (jika sudah diisi rekening / sedang diproses) --}}
                    ...
                    <div class="mt-3 pt-3 border-t border-red-200">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                @endphp
                                <span class="inline-block px-3 py-1 rounded-full text-[10px] font-bold border {{ $refundBadge }}">
                                    {{ $refundText }}
                                </span>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] text-red-600 mb-1">Dana Dikembalikan</p>
                                <p class="text-sm font-bold text-red-800">Rp {{ number_format($order->refund_amount, 0, ',', '.') }}</p>
                            </div>
                        </div>
                ...
                <div class="space-y-4">
                    ...
                    {{-- Produk dihapus --}}
                    @if(!$item->product)
                    <div class="flex gap-4 p-3 bg-gray-50 border border-dashed border-gray-300 rounded-lg">
                        <div class="w-16 h-16 bg-gray-200 rounded-md flex items-center justify-center shrink-0">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ $item->quantity }}x • Rp {{ number_format($item->total, 0, ',', '.') }}</p>
                        </div>
                        <div class="text-right flex flex-col justify-center">
                            <span class="text-sm font-bold text-gray-400">Rp {{ number_format($item->total, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    @continue
                    @endif
                    ...
                    <div class="flex gap-4 p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-[#8B4513] font-bold mt-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm h-full flex flex-col">
                        ...
                        <div class="space-y-2 text-xs text-gray-600 flex-1">
                            <div class="flex justify-between">
                                <span>Subtotal</span>
                                <span class="font-bold text-gray-800">Rp {{ number_format($order->sub_total, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                        ...
                        <div class="border-t border-gray-100 mt-3 pt-3 flex justify-between items-center">
                            ...
                            <span class="text-base font-bold text-[#0F4C20]">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                        </div>
            ...
            {{-- Action Buttons --}}
            <div class="bg-gray-50 px-5 py-4 border-t border-gray-100 flex justify-between gap-3 items-center flex-wrap">
                ...
                <button @click="open = !open"
                    ...
                    <x-heroicon-m-chevron-down class="w-4 h-4 transition-transform duration-300" ::class="open ? 'rotate-180' : ''" />
                </button>

                {{-- Right Actions --}}
                <div class="flex gap-3 items-center">

                    {{-- ✅ Tombol Isi Rekening (shortcut di action bar) --}}
                    @if($order->status === 'cancelled' && $order->refund && $order->refund->status === 'needs_bank_info')
                    <button @click="open = true"
                        class="px-5 py-2.5 rounded-lg bg-yellow-500 text-white text-sm font-bold hover:bg-yellow-600 shadow-md transition flex items-center gap-2">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div x-data="{ showModal: false }">
                        <button @click="showModal = true"
                            ...
                            <span class="text-xs opacity-80">({{ floor($remainingMinutes / 60) }}j {{ $remainingMinutes % 60 }}m)</span>
                            @endif
                        </button>
                        ...
                        <div x-show="showModal" x-cloak @click.away="showModal = false"
                            ...
                            <div @click.stop
                                ...
                                <form action="{{ route('profile.orders.cancel', $order->id) }}" method="POST">
                                    ...
                                    <div class="mb-4">
                                        <label class="block text-sm font-semibold text-gray-700 mb-2">Alasan Pembatalan (Opsional)</label>
                                    ...
                                    <div class="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                                        <div class="flex gap-2 mb-2">
                                            ...
                                            <div>
                                                <p class="text-sm font-semibold text-yellow-800 mb-1">Pengembalian Dana</p>
                                                <p class="text-xs text-yellow-700">Pesanan sudah dibayar. Dana akan dikembalikan setelah dikurangi biaya admin.</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-4 p-4 bg-gray-50 rounded-lg space-y-2 text-sm">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-semibold text-gray-900">Rp {{ number_format($orderAmount, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Biaya Admin (5%)</span>
                                            <span class="font-semibold text-red-600">- Rp {{ number_format($adminFee, 0, ',', '.') }}</span>
                                        </div>
                                        <hr class="border-gray-300">
                                        <div class="flex justify-between">
                                            <span class="font-bold text-gray-900">Dana Dikembalikan</span>
                                            <span class="font-bold text-green-600 text-base">Rp {{ number_format($refundAmount, 0, ',', '.') }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-4 space-y-3">
                                        ...
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Nama Pemilik Rekening <span class="text-red-500">*</span></label>
                                            <input type="text" name="account_holder" required
                        ...
                        <div x-show="showModal" x-cloak @click.away="showModal = false"
                            ...
                            <div @click.stop class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                                ...
                                <p class="text-sm text-gray-600 mb-6">
                                    Pastikan Anda sudah menerima pesanan dalam kondisi baik. Setelah konfirmasi, dana akan diteruskan ke penjual.
                                </p>
        ...
        @empty
        <div class="flex flex-col items-center justify-center py-16 bg-white rounded-xl border border-gray-200 border-dashed">
