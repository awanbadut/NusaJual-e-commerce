...
<a href="{{ route('produk.show', is_object($item) ? $item->id : 1) }}" class="group block h-full">
    ...
    <div
        ...
        <div class="h-[178px] w-full bg-gray-100 overflow-hidden shrink-0 relative">
            ...
            {{-- @if(is_object($item) && $item->price < 50000) <div
                class="absolute top-2 right-2 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full">
                Promo
        </div>
        @endif --}}
    </div>
        ...
        <div class="flex items-center gap-2.5 mt-auto">
            <div class="flex items-baseline gap-[1px]">
                <span class="text-[#8B4513] font-bold text-base tracking-tight">
                    Rp {{ number_format(is_object($item) ? $item->price : str_replace('.','', $item['price']), 0, ',',
                    '.') }}
                </span>
                <span class="text-xs font-medium text-[#8B4513]">/{{ is_object($item) ? $item->unit : 'Kg' }}</span>
            </div>

            @if(!is_object($item) && isset($item['old']) && $item['old'])
